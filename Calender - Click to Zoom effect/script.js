...
const TransformUtility = {
    calculateX: (index, calendarWidth, boxSize, scale, gridGap) => {
        ...
        const columnIndex = index % 7,
            columnIndexDiff = columnIndex - startingColumnIndex;
        const xPaddingPercent = ((gridGap * columnIndexDiff) / calendarWidth) * 100,
    ...
    calculateY: (index, calendarHeight, boxSize, scale, gridGap) => {
        const startingRowIndex = 2,
            startingYPercent = -50;
        const rowIndex = Math.floor(index / 7),
            rowIndexDiff = rowIndex - startingRowIndex;
        const yPaddingPercent = ((gridGap * rowIndexDiff) / calendarHeight) * 100,
...
const DateBox = (props) => {
    const today = new Date("2022-09-02T12:00:00"),
...
const Calendar = () => {
    ...
    React.useEffect(() => {
        if (ref) {
            const box = document.getElementById("date-2022-09-01");
            const boxSize = {
    ...
    const getStyles = () => {
        ...
        if (calendarSize && boxSize && selectedDate) {
            const scale = 4,
                gridGap = parseInt(TransformUtility.getGridGap());
            const x = TransformUtility.calculateX(selectedDate.index, calendarSize.width, boxSize.width, scale, gridGap),
